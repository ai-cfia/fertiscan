...
async def test_update_user_me(client: TestClient, db: AsyncSession) -> None:
...
async def test_update_user_me_email_same(client: TestClient, db: AsyncSession) -> None:
...
async def test_update_password_me(client: TestClient, db: AsyncSession) -> None:
    """Test updating current user password."""
    user = await create_random_user(db)
    await db.commit()
    old_password = random_lower_string()
    user_in_update = UserUpdate(password=old_password)
