...
def _configure_benchmark_paths() -> None:
...
async def run_benchmark_async() -> None:
    ...
    _configure_benchmark_paths()

    readiness = prescript()
    if not readiness.ok:
        _log_prescript_result(readiness)
        raise SystemExit(1)
    logger.info("Compliance benchmark prerequisites met.")
    ...
    logger.info("Loaded ground truth for %s label files.", len(ground_truth_lookup))
...
def _log_prescript_result(readiness: PrescriptResult) -> None:
    logger.error("Prerequisites not met. Please review the following issues:")
    ...
    for file_path in readiness.invalid_data_files:
        logger.error("Invalid data file: %s", file_path)
    for file_path in readiness.missing_ground_truth_files:
        logger.error("Missing ground truth file: %s", file_path)
